...
// short-link redirect route must be mounted at root so /:slug works
...
app.get('/api/links/public', authenticate, async (req, res) => {
  try {
    const links = await Link.find({ visibility: 'public', status: 'active' })
    .sort({ createdAt: -1 })
    .limit(500) // Limit to prevent overwhelming the client
    .select('_id slug title clicks createdAt ownerEmail showPreview isOneTime maxClicks collection status');
...
app.get('/api/links/:slug', async (req, res) => {
  try {
    ...
    let responseLink = link.toObject();
    if (responseLink.password) {
      responseLink.hasPassword = true;
      delete responseLink.password;
      delete responseLink.targetUrl;
      delete responseLink.conditionalRedirect;
    } else {
      responseLink.hasPassword = false;
      delete responseLink.password;
    }
...
app.post('/api/links/:slug/verify-password', async (req, res) => {
  try {
    const { slug } = req.params;
    ...
    if (!link.password) {
      return res.status(400).json({ message: 'Link does not require a password' });
    }
    ...
    return res.status(200).json({ targetUrl: link.targetUrl });
  } catch (err) {
...
app.put('/api/links/:id', authenticate, async (req, res) => {
  try {
    ...
    if (visibility !== undefined) {
      link.visibility = visibility === 'private' ? 'private' : 'public';
    }
...
io.on('connection', (socket) => {
  ...
  socket.on('join-admin-room', async (token) => {
    try {
      ...
      console.log('🔌 Unauthorized admin socket attempt:', socket.id);
    }
  });
