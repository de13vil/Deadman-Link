...
async function computeLinkSafetyForUrl(rawUrl) {
  ...
  // AI Scanner via Gemini REST API if KEY is present
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${geminiKey}`, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `Analyze this URL for safety: "${rawUrl}". You are a cybersecurity expert. Return a JSON object ONLY with the following exact keys: "score" (a number from 0 to 100 where 100 is extremely dangerous and phishing/malware, 0 is perfectly safe), "reasons" (an array of strings explaining why it's dangerous, empty if safe), "verdict" (a string: "low", "medium", or "high" risk). No markdown, no formatting, just pure JSON.`
            }]
          }]
        })
      });
      ...
      if (response.ok) {
        ...
        if (textResponse) {
          ...
          let verdict = parsed.verdict || 'low';
        ...
        console.warn(`[AI Scanner] Gemini API failed with status ${response.status}. Falling back to heuristic.`);
      }
    } catch (err) {
      console.warn('[AI Scanner] Failed to parse Gemini output, falling back to heuristic:', err.message);
    }
  }
