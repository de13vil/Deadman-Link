...
const RedirectHandler = () => {
    ...
    const fetchLink = async () => {
      try {
        ...
        timeoutId = setTimeout(() => {
          if (isMounted) {
            window.location.href = `${REDIRECT_BASE}/${slug}`;
          }
        }, 800);
      } catch (err) {
        if (!isMounted) return;
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      const pwdQuery = `?pwd=${encodeURIComponent(passwordInput)}`;

      if (linkData.showPreview) {
        setLinkData({ ...linkData, targetUrl: res.data.targetUrl, pwdQuery });
        setStatus('preview');
      } else {
        setStatus('redirect');
        setTimeout(() => {
          window.location.href = `${REDIRECT_BASE}/${slug}${pwdQuery}`;
        }, 800);
      }
    } catch (err) {
          ...
          <div className="space-y-3">
            <Button
              onClick={() => {
                setStatus('redirect');
                setTimeout(() => {
                  window.location.href = `${REDIRECT_BASE}/${slug}${linkData.pwdQuery || ''}`;
                }, 800);
              }}
            >
